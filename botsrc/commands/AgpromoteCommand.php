<?php
     ...
     * Agpromote command
     *
     * Promotes a user to an agent
     */
    class AgpromoteCommand extends UserCommand
    {
        /**
         * @var string
         */
        protected $name = 'Agpromote';

        /**
         * @var string
         */
        protected $description = 'Allows a main operator to promote a user to agent';

        /**
         * @var string
         */
        protected $usage = '/agpromote';
        ...
        public function execute()
        {
            ...
            if ($UserStatus->IsBlacklisted == true)
            {
                return Request::sendMessage([
                    ...
                    "reply_to_message_id" => $this->getMessage()->getMessageId(),
                    "parse_mode" => "html",
                    "text" => LanguageCommand::localizeChatText($this->WhoisCommand, "This user is blacklisted and cannot be made an agent")
                ]);
            }
            ...
            if ($UserStatus->IsAgent == true)
            {
                return Request::sendMessage([
                    ...
                    "reply_to_message_id" => $this->getMessage()->getMessageId(),
                    "parse_mode" => "html",
                    "text" => LanguageCommand::localizeChatText($this->WhoisCommand, "This user is already an agent")
                ]);
            }
            ...
            return Request::sendMessage([
                ...
                "reply_to_message_id" => $this->getMessage()->getMessageId(),
                "parse_mode" => "html",
                "text" => LanguageCommand::localizeChatText($this->WhoisCommand, "Success, this user is now an agent")
            ]);
        }
        ...
        public function displayUsage(Message $message, string $error="Missing parameter")
        {
            return Request::sendMessage([
                ...
                "text" =>
                    ...
                    "   <b>/agpromote</b> [PTID/ID/Username]\n".
                    LanguageCommand::localizeChatText($this->WhoisCommand, "For further instructions, send /help agpromote")
            ]);
        }
    }